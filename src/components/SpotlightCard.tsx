...
export default function SpotlightCard({
  ...
  return (
    <motion.div
      ...
      className={`group relative overflow-hidden rounded-[28px] border border-white/80 bg-white/95 transition-shadow duration-300 hover:shadow-[0_35px_90px_rgba(0,0,0,0.13)] md:bg-white/80 md:backdrop-blur-xl md:duration-500 ${className}`}
    >
      ...
      <div
        className="pointer-events-none absolute -inset-px hidden transition-opacity duration-300 md:block"
        style={{
      ...
      <div
        className="pointer-events-none absolute -inset-px hidden rounded-[28px] border border-black/20 transition-opacity duration-300 md:block"
        style={{
          opacity: mousePos.opacity,
      ...
      <div className="relative z-10 flex h-full flex-col justify-between md:[transform:translateZ(18px)]">
        {children}
      </div>
    </motion.div>
  );
