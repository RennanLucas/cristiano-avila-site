...
export default function BlogPreview() {
  ...
  return (
    <section id="blog" className="py-24 lg:py-32 bg-[#F8F8F5] relative">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="space-y-4 max-w-2xl">
            <div className="inline-flex items-center gap-3">
              ...
              <span className="tag-gold text-xs">BLOG & ARTIGOS CLÍNICOS</span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {articles.map((article, idx) => (
            <motion.article
              ...
              className="group bg-white rounded-2xl overflow-hidden border border-[#18363B]/8 shadow-sm hover:shadow-luxury-hover hover:-translate-y-1.5 transition-all duration-400 flex flex-col justify-between"
            >
              <div>
                {/* Article Image */}
                <div className="relative aspect-[16/10] w-full overflow-hidden bg-[#18363B]">
                  <img
                  ...
                  <span className="absolute top-4 left-4 text-[10px] uppercase tracking-wider font-semibold bg-white/95 text-[#18363B] px-3 py-1 rounded-full shadow-sm">
                    {article.category}
                  </span>
                </div>
                ...
                <div className="p-7 space-y-4">
                  <div className="flex items-center justify-between text-xs text-[#667174]">
                    <span>{article.date}</span>
                    <span>{article.readTime}</span>
                  </div>

                  <h3 className="font-serif text-xl sm:text-2xl text-[#18363B] font-semibold leading-snug group-hover:text-[#B7925A] transition-colors">
                    <Link href={`/blog/${article.slug}`}>
                      {article.title}
                    </Link>
                  </h3>

                  <p className="text-[#667174] text-sm leading-relaxed font-light line-clamp-3">
                    {article.excerpt}
                  </p>
                </div>
              </div>
